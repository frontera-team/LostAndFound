...
async function submit(): Promise<void> {
  ...
  const res = await api.request(`/api/announcements/${id}/replies`, { method: 'POST', json: { message: text } });
  if (res.ok) {
...
<template>
  <BaseModal v-model="modelValue" title="ОТКЛИКНУТЬСЯ">
    <form @submit.prevent="submit">
      <div class="input-group">
        ...
        <textarea
          ...
          required
          maxlength="4000"
          placeholder="Опишите, что известно о находке или как связаться…"
        ></textarea>
      </div>
